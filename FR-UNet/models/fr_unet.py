...
class FR_UNet(nn.Module):
    '''
    The way the indexing works is: block{level(top-bottom)}{shift to left or right from the center} . '_' indicates shift left
    and no '_' means shift right
        For example:
            block1_1 : level 1, 1 shift left (in the paper, it would be x02)
            block11: level 1, 1 shift right
            block30: level 3, 0 shift (central node: x23 in the paper)         
    '''
